...
public:
    ...
    static json find_matching_parent(json &haystack, json const &needle, std::function<bool (json const &)> matcher);
    static json find_tabbed(json &haystack, json const &needle);
    static json get_focused_child(json &haystack, size_t depth = SIZE_MAX);
    static json get_delta_child(json const &container, int delta, bool wrap);
    ...
    json find_matching_parent(json const &element, std::function<bool (json const &)> matcher)
    {
        return find_matching_parent(root, element, matcher);
    }
